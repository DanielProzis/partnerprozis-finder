...
export default function Dashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      <header style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '0 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 60, position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 34, height: 34, background: 'var(--accent)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: 16 }}>P</div>
          <span style={{ fontWeight: 600, fontSize: 16 }}>Partner Finder</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{user?.email}</span>
          <button className="btn-secondary" style={{ padding: '6px 14px', fontSize: 13 }} onClick={() => { supabase.auth.signOut(); router.push('/login') }}>Sair</button>
        </div>
      </header>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '2rem' }}>
        <div style={{ display: 'flex', gap: 4, marginBottom: '1.5rem', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 4, width: 'fit-content' }}>
          {([['search', '🔍 Pesquisar'], ['leads', `📋 Leads (${leads.length})`]] as const).map(([t, label]) => (
            <button key={t} onClick={() => setTab(t as Tab)} style={{ padding: '8px 20px', borderRadius: 8, fontSize: 14, fontWeight: 500, background: tab === t ? 'var(--accent)' : 'transparent', color: tab === t ? 'white' : 'var(--text-muted)', border: 'none' }}>{label}</button>
          ))}
        </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 16 }}>
                ...
                <div>
                  ...
                  <select value={partnerType} onChange={e => setPartnerType(e.target.value as PartnerType)} style={{ width: '100%', padding: '10px 14px', borderRadius: 'var(--radius)', border: '1.5px solid var(--border)', background: 'var(--bg)' }}>
                    {Object.entries(PARTNER_TYPES).map(([k, v]) => <option key={k} value={k}>{v.emoji} {v.label}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>Cidade (opcional)</label>
                  <input value={city} onChange={e => setCity(e.target.value)} placeholder="Porto, Lisboa, Madrid..." onKeyDown={e => e.key === 'Enter' && handleSearch()} style={{ width: '100%', padding: '10px 14px', borderRadius: 'var(--radius)', border: '1.5px solid var(--border)', background: 'var(--bg)' }} />
                </div>
              </div>
                    ...
                    <div key={r.google_place_id} className="card" style={{ padding: '1.25rem', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                          ...
                          <div style={{ flex: 1 }} />
                          <button onClick={() => !r.saved && saveLead(r)} disabled={r.saved || saving === r.google_place_id} style={{ padding: '6px 14px', borderRadius: 8, fontSize: 13, fontWeight: 500, background: r.saved ? 'var(--accent-light)' : 'var(--accent)', color: r.saved ? 'var(--accent)' : 'white', border: r.saved ? '1px solid var(--accent)' : 'none', cursor: r.saved ? 'default' : 'pointer' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: 12 }}>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {(['all', 'to_contact', 'contacted', 'partner', 'not_interested'] as const).map(s => (
                  <button key={s} onClick={() => setLeadsFilter(s)} style={{ padding: '6px 14px', borderRadius: 8, fontSize: 13, fontWeight: 500, background: leadsFilter === s ? 'var(--text)' : 'var(--surface)', color: leadsFilter === s ? 'white' : 'var(--text-muted)', border: '1px solid var(--border)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: selectedLead ? '1fr 380px' : '1fr', gap: 16, alignItems: 'start' }}>
                <div style={{ display: 'grid', gap: 10 }}>
                  {filteredLeads.map(lead => (
                    <div key={lead.id} className="card" onClick={() => { setSelectedLead(lead); setNotes(lead.notes || '') }} style={{ padding: '1rem 1.25rem', cursor: 'pointer', border: selectedLead?.id === lead.id ? '1.5px solid var(--accent)' : '1px solid var(--border)' }}>
                ...
                {selectedLead && (
                  <div className="card" style={{ padding: '1.5rem', position: 'sticky', top: 76 }}>
                    ...
                    <div style={{ marginBottom: '1rem' }}>
                      ...
                      <select value={selectedLead.status} onChange={e => updateLead(selectedLead.id, { status: e.target.value as any })} style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1.5px solid var(--border)', background: 'var(--bg)', fontSize: 14 }}>
                    ...
                    <div style={{ marginBottom: '1rem' }}>
                      ...
                      <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Adiciona notas sobre este contacto..." style={{ width: '100%', padding: '10px 12px', borderRadius: 8, height: 100, border: '1.5px solid var(--border)', background: 'var(--bg)', resize: 'vertical', fontSize: 14 }} />
                      <button className="btn-primary" onClick={() => updateLead(selectedLead.id, { notes })} style={{ marginTop: 8, width: '100%', padding: '8px' }}>Guardar notas</button>
                    </div>
                    <button onClick={() => deleteLead(selectedLead.id)} style={{ width: '100%', padding: '8px', borderRadius: 8, fontSize: 13, background: 'var(--danger-light)', color: 'var(--danger)', border: '1px solid #f5c6c4' }}>
                      🗑️ Remover lead
                    </button>
                  </div>
                )}
